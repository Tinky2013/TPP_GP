...
if __name__ == "__main__":
    ...
    intensity=np.vectorize(lambda x: 50*np.exp(np.sin(2*np.pi*x)))  # intensity相当于两个函数，可以接受两个向量
    bound=50*3

    PP=simPP(intensity, bound)  # 得到一系列非齐次泊松的样本
    N=PP.size

    Y=intensity(X)
    plt.plot(X,Y,'b-')          # 绘制intensity曲线
    plt.scatter(PP,np.zeros(N),marker='|')  # x轴上绘制泊松分布的样本
    plt.show()
