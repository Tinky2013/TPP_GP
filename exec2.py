...
with pm.Model() as model:
    ...
    Lambda = tt.exp(f[:split]) + tt.dot(decayKernel(y_train)**beta, y_train) * alpha
...
par_dt.to_csv("par_dt2.csv",index=False)
...
with model:
    Lambda = tt.exp(f[split:]) + tt.dot(decayKernel(y_test)**beta, y_test) * alpha
...
train_result_dt.to_csv("train_result2.csv",index=False)
test_result_dt = pd.DataFrame(forecasts_for_test).T
test_result_dt.to_csv("test_result2.csv",index=False)
